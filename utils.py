...
def show_plots(img1,img2,img4,img_depth,scaled,n_disp,method=1):
    text = "Disparity and Depth Map with Simple block matching"
    ...
    ax4.imshow(img4 , cmap = 'magma')
    plt.axis('off') 
    ax4.set_title("Disparity Map - magma Color Scale", fontsize=10)
    ax5 = fig.add_subplot(4,2,5)
    ax5.imshow(scaled,cmap='gray')
    plt.axis('off') 
    ax5.set_title('Disparity Map Scaled')
    ax6 = fig.add_subplot(4,2,6)
    ax6.imshow(img4,cmap='magma')
    plt.axis('off') 
